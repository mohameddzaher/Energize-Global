...
export default function UserManagement() {
  ...
  const getPermissionsText = (user: User) => {
    const permissions = user.bookingPermissions || {
      smallRoom: true,
      largeRoom: false,
    };
              ...
              <tbody className="bg-gray-800/50 divide-y divide-gray-700">
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getPermissionsColor(
                          user
                        )}`}
                      >
